...
std::string format_error(const std::string& base, std::string source, int index) {
  std::ostringstream s;
  int counter = 0;
  int line = 1;
  int column = 0;
  ...
  while (size_t(counter) < source.size()) {
...
int lex_whitespace(std::string raw_json, int index) {
  while (std::isspace(raw_json[index])) {
    if (size_t(index) == raw_json.length()) {
      break;
    }
...
std::tuple<JSONToken, int, std::string> lex_keyword(std::string raw_json,
  ...
  if (size_t(index - original_index) == keyword.length()) {
    token.value = keyword;
  }
  return {token, index, ""};
}
...
std::tuple<JSONToken, int, std::string> lex_null(std::string raw_json,
                                                 int index) {
  return lex_keyword(std::move(raw_json), "null", JSONTokenType::Null, index);
}

std::tuple<JSONToken, int, std::string> lex_true(std::string raw_json,
                                                 int index) {
  return lex_keyword(std::move(raw_json), "true", JSONTokenType::Boolean, index);
}

std::tuple<JSONToken, int, std::string> lex_false(std::string raw_json,
                                                  int index) {
  return lex_keyword(std::move(raw_json), "false", JSONTokenType::Boolean, index);
}
...
std::tuple<std::vector<JSONToken>, std::string> lex(const std::string& raw_json) {
  ...
  for (size_t i = 0; i < raw_json.length(); i++) {
    if (auto new_index = lex_whitespace(raw_json, int(i)); int(i) != new_index) {
      i = new_index - 1;
      continue;
    }
    ...
    for (auto lexer : generic_lexers) {
      if (auto [token, new_index, error] = lexer(raw_json, int(i)); int(i) != new_index) {
        if (error.length()) {
          return {{}, error};
        }
...
std::string format_parse_error(const std::string& base, const JSONToken& token) {
  std::ostringstream s;
...
parse_array(std::vector<JSONToken> tokens, int index) {
  ...
  while (size_t(index) < tokens.size()) {
    auto t = tokens[index];
...
std::tuple<JSONValue, std::string> parse(const std::string& source) {
...
std::string deparse(JSONValue v, const std::string& whitespace) {
  switch (v.type) {
